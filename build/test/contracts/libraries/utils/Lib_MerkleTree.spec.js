...
describe('Lib_MerkleTree', () => {
    ...
    describe('verify', () => {
        describe('when total elements is zero', () => {
            const totalLeaves = 0;
            it('should revert', async () => {
                await setup_1.expect(Lib_MerkleTree.verify(hardhat_1.ethers.constants.HashZero, hardhat_1.ethers.constants.HashZero, 0, [], totalLeaves)).to.be.revertedWith('Lib_MerkleTree: Total leaves must be greater than zero.');
            });
        });
        describe('when an index is out of bounds', () => {
            ...
            const index = 2;
            it('should revert', async () => {
                await setup_1.expect(Lib_MerkleTree.verify(hardhat_1.ethers.constants.HashZero, hardhat_1.ethers.constants.HashZero, index, [], totalLeaves)).to.be.revertedWith('Lib_MerkleTree: Index out of bounds.');
            });
        });
        describe('when total siblings does not match provided total leaves', () => {
            ...
            it('should revert', async () => {
                await setup_1.expect(Lib_MerkleTree.verify(hardhat_1.ethers.constants.HashZero, hardhat_1.ethers.constants.HashZero, 0, siblings, totalLeaves)).to.be.revertedWith('Lib_MerkleTree: Total siblings does not correctly correspond to total leaves.');
            });
        });
