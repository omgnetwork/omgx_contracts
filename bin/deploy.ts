...
process.env.HARDHAT_NETWORK = 'rinkeby' // "rinkeby" here is an arbitrary name. only used for CI.
...
const main = async () => {
  ...
  const contracts: any = dirtree(
    ...
    const artifact = require(path.resolve(__dirname, `../deployments/rinkeby/${child.name}`))
